<?php
    ...
    if($loggedIn == 1){
        ...
        $id = $_POST['id'];
        $query = qM("SELECT m.id,m.name,m.gym_name,m.public,p.user_id,p.date_update,p.pic_path,p.pic_desc FROM `pictures` AS `p` LEFT JOIN `members` as `m` ON `p`.`user_id`=`m`.`id` WHERE `public`=1 OR `m`.`id`=$id OR `m`.`id` IN ($following2) ORDER BY `date_update` DESC LIMIT ".$load.",2");
    }
    else{
