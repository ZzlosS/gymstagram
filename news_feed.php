<?php
    ...
    if($loggedIn == 1){
        ...
        $query = qM("SELECT m.id,m.name,m.gym_name,p.user_id,p.date_update,p.pic_path,p.pic_desc FROM `pictures` AS `p` LEFT JOIN `members` as `m` ON `p`.`user_id`=`m`.`id`  WHERE `public`=1 ORDER BY `date_update` DESC LIMIT ".$load.",2");
    }
